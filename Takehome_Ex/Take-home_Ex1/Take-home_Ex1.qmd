...
Observation: 
The analysis of numerical variables through histograms and standardized box plots reveals key patterns in data distribution and variability. Most variables exhibit consistent dispersion after standardization, suggesting a well-balanced dataset. However, Weekly_Voyage_Count shows a discrete pattern, indicating specific count categories rather than a continuous spread. Variables such as Operational_Cost_USD and Revenue_per_Voyage_USD demonstrate high natural variance, which may impact business insights. Notably, the standardized box plot confirms the absence of extreme outliers, ensuring that no single variable disproportionately skews the dataset. These insights highlight a dataset that is well-structured, with stable numerical distributions.

In terms of the categorical variables, they are well-distributed with minimal missing data. Diesel, HFO, and Steam Turbine dominate engine types, while maintenance status is balanced across "Critical," "Fair," and "Good." Route and ship types are diverse, with Tankers and Bulk Carriers being more frequent. Weather conditions are mainly "Calm," "Moderate," or "Rough." This ensures a well-structured dataset for analysis.

Overall, the dataset is well-balanced, with stable numerical distributions and diverse categorical attributes, making it suitable for K-Means clustering analysis. The well-spread numerical features and natural variability provide a strong foundation for identifying meaningful clusters in operational performance, travel patterns, and maintenance risk levels.
...
```
Observation: 
The cost range for all engine types is generally similar, ranging from \$0 to \$500,000. The whisker lengths of the box plots are also comparable, indicating that the gap between the lowest and highest operational costs remains relatively consistent. Additionally, the median cost is close to \$250,000.

Diesel and HFO engines have a relatively smaller IQR, suggesting that their operational costs are more stable. In contrast, Steam Turbine has a more dispersed density distribution, indicating that its operational costs may be influenced by a greater number of factors.

The density plot suggests that Steam Turbine may exhibit higher cost variability compared to other engine types.
...
Observation: 
Operational costs show noticeable variation across different factors. Diesel and HFO engines exhibit a wider cost distribution, while steam turbines have a more concentrated range. Additionally, long-haul routes and rough weather conditions are linked to higher operational expenses, indicating external environmental influences on cost.

#### Revenue per Voyage(USD) vs. Different Catagorical Variables
...
Through the PCA Loadings Heatmap, we identified the key variables that significantly impact PC3, PC4, and PC5. Then, by comparing with the 3D clustering results, we determined which variables have the greatest influence on each cluster.
...
[Rstudio/Cheatsheets](https://github.com/rstudio/cheatsheets/blob/main/data-visualization.pdf)
