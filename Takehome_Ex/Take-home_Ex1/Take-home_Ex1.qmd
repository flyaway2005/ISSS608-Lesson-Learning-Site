...
pca_melt <- melt(cluster_pca_summary, id.vars = "Cluster")

ggplot(pca_melt, aes(x = variable, y = value, group = Cluster, color = Cluster)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  labs(title = "Cluster Projection on PCA Axes", x = "Principal Component", y = "Mean Value") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
```{R}
library(dplyr)
...
# 確保變數名稱與 One-Hot 編碼的結果一致
encoded_categorical_vars <- pca_data %>%
    select(Cluster, starts_with("Engine_Type"), 
                  starts_with("Route_Type"), 
                  starts_with("Ship_Type"), 
                  starts_with("Weather_Condition"), 
                  starts_with("Maintenance_Status"))

# 確保數據正確
print(colnames(encoded_categorical_vars))

# 計算各類別變數的均值作為比例
categorical_proportion <- encoded_categorical_vars %>%
    group_by(Cluster) %>%
    summarise(across(where(is.numeric), mean, na.rm = TRUE)) %>%
    pivot_longer(cols = everything(), names_to = "Variable", values_to = "Proportion")

# 輸出數據檢查
print(categorical_proportion)

# 視覺化結果
ggplot(categorical_proportion, aes(x = as.factor(Cluster), y = Proportion, fill = Variable)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = "Proportion of Encoded Categorical Variables in Each Cluster",
