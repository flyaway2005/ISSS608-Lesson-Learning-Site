...
```
Conclusion of 5-1: k value may be 3
...
Ship_pca_df$Cluster <- as.factor(kmeans_result$cluster) 
...
```{R}
library(plotly)
p <- ggplot(Ship_pca_df, aes(x = PC2, y = PC3, color = Cluster)) +
...
```{R}
p <- ggplot(Ship_pca_df, aes(x = PC4, y = PC5, color = Cluster)) +
  geom_point(alpha = 0.6, size = 2) +
  theme_minimal() +
...
We select PC 1-5 to perform PCA 3D visualization and k means
```{R}
selected_pcs <- pca_result_new$x[, 1:5]  
...
```{R}
pca_df <- data.frame(pca_result_new$x[, 1:5])
colnames(pca_df) <- c("PC3", "PC4", "PC5")

plot_ly(pca_df, x = ~PC3, y = ~PC4, z = ~PC5, type = "scatter3d", mode = "markers")
```
```{R}
#| include: false
dim(pca_result_new$rotation[, 1:5])
```
```{R}
dim(selected_pcs_df)
head(selected_pcs_df)
```


### 5-8. K means result
Check appropriate K by Silhouette Score
```{R}
library(cluster)

for (k in 2:6) {  # 測試 K=2 到 K=6
  kmeans_model <- kmeans(pca_df, centers = k, nstart = 25)
  silhouette_score <- silhouette(kmeans_model$cluster, dist(pca_df))
  avg_sil <- mean(silhouette_score[, 3])
  print(paste("K =", k, "Silhouette Score:", avg_sil))
}
```
The result show that k=5 is the best choice, so we perform k means clustering with k = 5.


```{R}
#| include: false
set.seed(123)
kmeans_result <- kmeans(pca_df[, 1:5], centers = 5, nstart = 25)
...
The plot:
```{R}
plot_ly(pca_df, x = ~PC3, y = ~PC4, z = ~PC5,
        color = ~Cluster, colors = c("red", "blue", "green", "purple", "grey"),
        type = "scatter3d", mode = "markers",
        marker = list(size = 5)) %>%
  layout(title = "K-Means Clustering Visualization (PC3 vs PC4 vs PC5)")
```
```{R}
colnames(pca_df)
```
...
selected_pcs_df <- pca_df[, c("PC3", "PC4", "PC5", "Cluster")]

selected_pcs_matrix <- as.matrix(selected_pcs_df[, c("PC3", "PC4", "PC5")])


selected_pcs_matrix <- as.matrix(selected_pcs_df[, c("PC3", "PC4", "PC5")])

colnames(restored_data) <- colnames(Ship_selected)

restored_data <- as.data.frame(restored_data)

restored_data$Cluster <- selected_pcs_df$Cluster

head(restored_data)
dim(restored_data)

```
Calculate the mean of numerical variables:

```{R}
library(dplyr)

if(!"Cluster" %in% colnames(restored_data)) {
  stop("Error: 'Cluster' column not found in restored_data.")
}

numerical_summary <- restored_data %>%
  group_by(Cluster) %>%
...
```
```{R}
sapply(restored_data, class)
```
```{R}
# 確保某些變數為 factor
restored_data <- restored_data %>%
  mutate(across(c("Ship_Type_Fish Carrier", "Route_Type_Transoceanic", "Weather_Condition_Moderate"), as.factor))
...
# 確保 restored_data 包含 Cluster 變數
if(!"Cluster" %in% colnames(restored_data)) {
  stop("Error: 'Cluster' column not found in restored_data.")
}
...
# 確保至少有一個類別變數
categorical_vars <- restored_data %>% select(where(is.factor), Cluster)
if(ncol(categorical_vars) <= 1) {
  stop("Error: No categorical variables found in restored_data.")
}

# 計算類別變數在每個 Cluster 內的分佈
categorical_summary <- categorical_vars %>%
  pivot_longer(cols = -Cluster, names_to = "Variable", values_to = "Category") %>%
  group_by(Cluster, Variable, Category) %>%
