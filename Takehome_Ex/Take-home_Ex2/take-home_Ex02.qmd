...
```{r}
pacman::p_load(plotly, dplyr, scales, viridis, lubridate, ggthemes, gridExtra, readxl, knitr, data.table, CGPfunctions, ggHoriPlot, tidyverse, CGPfunctions)
```
...
import_long <- import_s %>%
  ...
  filter(Year == 2023 & !`Data Series` %in% c("Asia", "Europe", "North America" ))
...
# 轉換數據格式 (long format) 並篩選 2019 和 2023 年數據
import_long <- import_s %>%
  mutate(across(-`Data Series`, as.character)) %>%
  pivot_longer(cols = -`Data Series`, names_to = "Year", values_to = "Import_Value") %>%
  mutate(Year = as.integer(Year),
         Import_Value = as.numeric(Import_Value)) %>%
  filter(Year %in% c(2019, 2023)) # 只保留2019和2023年的數據
...
trade_2023 <- import_long %>%
  ...
  filter(`Data Series` %in% top_10_partners_2023) %>% # 只保留2023年前十大貿易夥伴
