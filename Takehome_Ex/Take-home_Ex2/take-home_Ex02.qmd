...
```
Step 3: Plot Cycle plot and make it interactive
...
total_combined <- i_total_market %>%
  ...
  mutate(Value = Value_import + Value_export, Type = "Total (Import + Export)") %>%
  select(Year, Month, Value, Type)

# Import, Export, Total
total_market <- bind_rows(
  i_total_market %>% mutate(Type = "Import"),
  e_total_market %>% mutate(Type = "Export"),
  total_combined
)
...
p <- ggplot(total_market, aes(x = Year, y = Value, group = interaction(Month, Type), color = Type, 
                              text = paste("Year:", Year, "<br>Value:", Value, "Million Dollars(SGD)", "<br>Type:", Type))) +
  geom_line(size = 0.8) +  
